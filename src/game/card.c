...
s32 HuCardCheck(void)
{
	...
	if(result == CARD_RESULT_READY || result == CARD_RESULT_WRONGDEVICE) {
		ret |= 1;
...
s32 HuCardWrite(CARDFileInfo *fileInfo, const void *addr, s32 length, s32 offset)
{
    ...
    SRDisable = HuSRDisableF;
    HuSRDisableF = TRUE;
    result = CARDWrite(fileInfo, addr, length, offset);
    HuSRDisableF = SRDisable;
	return result;
}
...
s32 HuCardWriteAsync(CARDFileInfo *fileInfo, const void *addr, s32 length, s32 offset)
{
	s32 result;
	if(WriteStatus) {
		while(!HuCardWriteCheck(&result)) {
			HuPrcVSleep();
		}
	}
	WriteStatus = TRUE;
	result = CARDWriteAsync(fileInfo, addr, length, offset, WriteCallBack);
	return result;
}
...
BOOL HuCardWriteCheck(s32 *result)
{
	if(WriteStatus) {
		return FALSE;
	}
...
s32 HuCardWriteIdle(CARDFileInfo *fileInfo, const void *addr, s32 length, s32 offset)
{
	s32 result = HuCardWriteAsync(fileInfo, addr, length, offset);
...
s32 HuCardDelete(s16 slot, const char *fileName)
{
	s32 result;
    HuSRDisableF = TRUE;
    result = CARDDelete(slot, fileName);
    HuSRDisableF = FALSE;
	return result;
}
